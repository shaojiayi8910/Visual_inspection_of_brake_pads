...
void CircleDetector::drawDetectedCircles(cv::Mat& image) {
    ...
    if (circleCenter1_.x && circleCenter2_.x) {
        cv::line(image, circleCenter1_, circleCenter2_, cv::Scalar(0, 0, 255), 10);
        ...
        cv::putText(image, distanceText, cv::Point(image.cols / 3, image.rows / 3), cv::FONT_HERSHEY_SIMPLEX, 4, cv::Scalar(0, 0, 255), 10);
    }
}
