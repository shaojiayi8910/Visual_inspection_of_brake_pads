...
DistanceResult calculate_length(const cv::Mat &image) {
    ...
    // 绘制线条
    cv::line(image_ROI, top_point, down_point, cv::Scalar(0, 0, 255), 2);

    // 将距离显示在图像上
    std::string distance_text = "Distance: " + std::to_string(result.distance) + " cm";
    cv::putText(image_ROI, distance_text, cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 2, cv::Scalar(0, 0, 255), 2);
